...
// times for a 4 KB band. The bands are RGB565_SWAPPED (big-endian in memory,
...
class Display {
 public:
  ...
  // Push `h` rows of `w` pixels (RGB565_SWAPPED) to rows y..y+h-1. Synchronous:
  // returns when the last byte has left the FIFO.
  void write(int y, int w, int h, const uint16_t *pixels);

  // Fill the whole panel with one RGB565_SWAPPED value
