...
| `render::Renderer` | 20,200 |
| SDK / pico-sdk / newlib / TinyUSB ほか | 約 7,250 |
| **合計** | **267,896 (261.6KB / 512KB)** |

`.text` は 289,648 バイト (4MB のフラッシュに対して十分小さい)。
...
RAM に載せる必要があるのは `.text` 193,248 + `.rodata` 77,900 + `.data` 9,724 +
`.bss` 267,896 = **548,768 バイト**で、512KB (524,288) を 24,480 超えている。
...
| | 三角形 | ピーク使用 | 三角形バッファ |
|---|---|---|---|
| 現在 (4 レイヤ、アリーナ 64KB) | 261 | **17,732B** | 54,464B |
...
| ShapoGFX | 変更 | ピーク使用 (実機バイト) |
|---|---|---|
| 8eff034 | `Triangle` 136→128B | — |
| b03055e | `SHAPOGFX3D_TEXTURE=0` で 128→92B | 25,056B |
| ba15719 | レコード可変長化 | 19,136B (−24%) |
| ba15719 | + レイヤ分け (下記) | **17,732B (−29%)** |
...
`buildScene()` は 4 つ開いている:

| レイヤ | 内容 | フラグ | 根拠 |
|---|---|---|---|
| 1 | 星 | `NO_DEPTH` | 1500FU の天球上。後ろに何も無く、互いに隠れない |
| 2 | スフィアのワイヤーフレーム | `NO_DEPTH` | 下記 |
...
実測: **RAM 116,916 / 327,680 (35.7%)** (帯バッファは実行時にヒープから取るので
この数字には入らない)、Flash 約 606KB / 3,342,336 (18.1%)。
ヒープに残るのは 210KB で、96KB アリーナだった頃の 178KB より 32KB 多い。
...
TRI 261 17/53K d0         三角形数 / バッファの使用量と総量 (KB) / 取りこぼし
SPN 34 d0 ARN 23K         スパンのピークと取りこぼし、アリーナの実使用量
...
- **ShapoGFX の更新のたびにシーン構築側のスタックが増えている。**
  `-fstack-usage` で最深経路 (`beginFrame` → `buildScene` → `buildSphere` →
  `subdivideFace` x7 → `emitEdge` → `putLine3` → `putPrimitive`) を足し上げた値:

  | | 最深経路 (静的) | 4096 までの残り |
  |---|---|---|
  | 8eff034 より前 | 2,272 | 1,824 |
  | 8eff034 | 2,440 | 1,656 |
  | b03055e | 2,528 | 1,568 |
  | ba15719 (現在) | 2,520 | 1,576 |

  ba15719 での実機実測 (`STK0`): **RP2350 2,532 / ESP32S3 3,540**。
  RP2350 は静的見積り 2,520 とよく一致する。ESP32S3 が 1,000 バイト多いのは
  FreeRTOS のタスク切り替えと Xtensa のレジスタウィンドウのぶんで、
  残りは 556 バイト。**ここが一番細い**。
  ...
  静的解析はインライン展開された呼び出し先や割り込みフレームを見落とすので、
  実測の `STK0` (過去に 3,552) の方が大きい。**更新のたびに `STK0` を見ること。**
