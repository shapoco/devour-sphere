...
| `render::Renderer` | 20,176 |
| SDK / pico-sdk / newlib / TinyUSB ほか | 約 7,250 |
| **合計** | **333,408 (325.6KB / 512KB)** |

`.text` は 284,472 バイト (4MB のフラッシュに対して十分小さい)。
`.bss` は ShapoGFX の更新では変わらない (アリーナは固定長の配列で、
その中の割り付けが変わるだけ)。フラッシュの方は 8eff034 で +2.7KB、
b03055e で +4.3KB 増えている。b03055e の増加は `PackedVertex` の
デコード経路のぶんで、`SHAPOGFX3D_TEXTURE=0` が返す 4.5KB を上回った
(`gfx3d.cpp` 単体: 28,256 → 36,368 → 31,812 バイト)。
...
アリーナの制約になるのはシーンが使うバイト数 (ピークで 30.4KB) ではなく、
そこから切り出される三角形バッファの容量の方。`buildScene()` は
`triCapacity` の残りを予算として配るので、足りないとまずエンティティが
フラグメント単位の細かい形を失い、次にスフィアのワイヤーフレームが間引かれる。

240x240、レベル 1〜7 を AI プレイヤーに回させて測ったピーク三角形数と容量
(容量は実機と同じ 32 ビットでの値。プロファイル表示の `TRI` と一致する):

| アリーナ | テクスチャ有り | 無し (実際のビルド) |
|---|---|---|
| 48KB | 193/251 | 193/350 |
| 64KB | 193/344 | 198/478 |
| 76KB | — | **263/574** ← 変曲点 |
| 96KB | 230/530 | **263/734** ← ESP32S3 |
| 128KB | 263/717 | **263/1064** ← RP2350 |
| 256KB | 263/1700 | 263/2429 |

変曲点は **76KB**。ここでシーンが本来の大きさ (263 三角形) に達し、
これ以上増やしても何も変わらない。現状 ESP32S3 で 2.8 倍、RP2350 で 4 倍の余裕がある。

ここに至る経緯:

- **ShapoGFX 8eff034** (レンダラの高速化) で `Triangle` が 136→128 バイトになり、
  同じアリーナが約 6% 多く入るようになった。変曲点は 112KB→104KB。
- **ShapoGFX b03055e** で `SHAPOGFX3D_TEXTURE=0` が入り、`Triangle` が 128→92 バイト、
  `Span` が 64→48 バイトに。変曲点は 104KB→76KB。
  **これで ESP32S3 の 96KB が初めてシーン全体を保持できるようになった**
  (ピーク 230→263 三角形)。

LOD は `triCapacity` から毎フレーム予算を引き直すので、この余裕は自動的に使われる。
定数の調整は不要で、`ARENA_SIZE` も据え置いている。
...
実測: **RAM 149,660 / 327,680 (45.7%)** (帯バッファは実行時にヒープから取るので
この数字には入らない)、Flash 約 599KB / 3,342,336 (17.9%)。
...
TRI 140/500 d0              アリーナ 96KB での容量 (現在の構成では 734)
...
TRI 263/1064 d0           三角形バッファの使用量と取りこぼし
SPN 44 d0 ARN 30K         スパンのピークと取りこぼし、アリーナの実使用量
...
- **ShapoGFX の更新のたびにシーン構築側のスタックが増えている。**
  ...
  | 8eff034 | 2,440 | 1,656 |
  | b03055e (現在) | 2,528 | 1,568 |

  b03055e ぶんの +88 は、`VertexBuffer` が 8→36 バイトになった (`packed` /
