...
|---|---|
| 3D アリーナ | 65,536 |
| `sim::Game` | 84,088 (Entity を 224B に詰める前は 95,360、上限を絞る前は 136,512) |
...
| レコード | バイト (ShapoGFX d538138 以降) | 88d2bb5 まで |
|---|---|---|
| フラット + 深度 | 60 | 64 |
| スムーズ + 深度 | 76 | 96 |
| フラット + `NO_DEPTH` | 48 | 52 |
...
この値は ShapoGFX の `Graphics3D::primitiveBytes()` で得られ、`buildScene()` の予算もそれを使う。
下の実測と変曲点は 88d2bb5 までの値 (レコードが小さくなったぶん、今は変曲点がさらに下にある。
同じ条件の M5Tab5 では 40KB → 24KB になった。impl/m5tab5/SPEC.md「アリーナの大きさ」)。
このボードは RAM に余裕があるのでアリーナは 64KB のまま。
...
スパンは ShapoGFX d538138 で 68B → 16B になったので、128 本で 8,704B → 2,048B。
...
- SDK の `RGB565` はメモリ上でバイトスワップされており、ShapoGFX の
  `RGB565_SWAPPED` とビット単位で一致する。変換は不要。
...
- ボタンの押下エッジは追いつき tick を跨いでも壊れない
  (`Game::tick()` が方向を含むボタンの前回値との差で押下を計算するので、
...
- **出力は GP1** (`XMC_PIN_AUDIO_OUT`、PWM スライス 0 チャネル B)。回路は
  ...
  サンプルは **DMA ペーシングタイマ** (sysclk × 1/10416 ≒ 24,001 Hz) で送る。
- **無音時は中央レベル** (AC 結合なのでアンプには何も届かない)。パックにはランプを付けない (`--ramp 0`)。
  サンプルレートは 24 kHz (ESP32S3 と共通、上記)。
...
- **サンプルレートは 24 kHz** (全ビルド共通)。PCM→PDM のハードウェアは fp = 960、fs = レート / 100 の
  補間器と、その整数商をオーバーサンプル比とするシグマデルタ変調器からなり、960 / (レート / 100) が
  整数でないと両者のレートが食い違って波形が崩れる (22.05 kHz では fs = 220、4.36 対 4 で、実機では
  テープの早送りのようなキュルキュルした音になった)。24000 (4) と 48000 (2) が使える。
  se_player.cpp は割り切れないレートを渡されたら fs = 480 の別構成 (Fpdm = 128 × レート) に逃がす。
- **パックは Web 版の se.bin** (s16、24 kHz) をそのまま使う。I2S に渡すのは s16 PCM なので変換不要で、
...
SPN 33 d0 ARN 22K L80     スパンのピークと取りこぼし、アリーナの実使用量、3D パイプラインに入れた線の本数
CMD 1.85  W 0.30          帯ごとの setWindow + 転送開始 / core0 が core1 を待った時間
XFR 16.64                 起動時に測った全画面転送 (ms)
...
- **シーン構築側のスタックは毎回動くので、更新のたびに見ること。**
  ...
  | ワイヤーフレーム書き換え後 | 2,596 | 2,340 | 3,252 |
  | ShapoGFX 28d841f | | | 3,120 (遊んでいる最中) |
  | core 3.4 のベンチマーク | | 2,260 | 3,360 |
  | core 3.5 (ShapoGFX e0f9662) のベンチマーク | | 2,260 | 3,360 |
  | core 3.6 (ShapoGFX 4f15411) のベンチマーク (現在) | | **2,260** | **3,360** |
  ...
  ShapoGFX d538138 では `putPrimitive()` 以下が深くなった (三角形のセットアップが分かれ、
  ガードバンドでクリップする経路が加わった)。`-fstack-usage` の和で `putPrimitive` から下は
  M33 で 312 → 776、ESP32S3 (xtensa-esp32s3-elf 14.2、-O2) で 384 → 896 バイト。
  ただし星とワイヤーフレームが 2D に移った今、網の再帰は `putPrimitive()` を通らず、
  最深経路は今もその再帰 (`beginFrame` → `buildScene` → `buildSphere` → `traverseSphere` →
  `subdivideFace` x7 → `emitSplitEdge` → `emitChord` → `addWireSegment` → `projectQ`、
  ...
  `beginFrame` から下の網の再帰 (2,624) には届かないので `STK0` は変わらない見込みで、実機でも両ボードとも変わらなかった (core 3.5 のベンチマーク)。
...
タイトル画面で **B を 3 秒** 押し続けると始まる (core/SPEC.md「ベンチマーク」)。RP2350 は 60Hz で 1,980 フレーム、
ESP32S3 は 30Hz で 990 フレーム。結果は画面に出て、ESP32S3 はシリアル (`ds::traceLine()`、115200) にも同じ行を出す。
A で次のページ、B で閉じる。値の意味はデバッグ表示と同じ (`DMA` は帯の転送完了待ち、`CMD` は帯ごとの
`setWindow` と転送開始、`W` は core0 が core1 を待った時間、`XFR` は起動時の全画面転送、`STK0` / `STK1` は
スタックのピーク)。このボードのオーバーレイはフェーズを分けないので、フェーズ計測の時計
(`xmc::getTimeUs()`) はベンチマークの実行中だけ入れる。
...
```
BENCHMARK v3.4  XIAMOCON RP2350  240x240 T60  XFR 17.6  STK0 2260  STK1 844
         QUIET   CROWD  FLIGHT
FPS      37.5    35.1    40.3
FRM      26.5    28.5    24.8
FRM^    27.8?    31.0    28.1
...
```
BENCHMARK v3.4  XIAMOCON ESP32S3  240x240 T30  XFR 25.6  STK0 3360  STK1 1684
         QUIET   CROWD  FLIGHT
FPS      34.9    33.3    36.2
FRM      28.6    30.0    27.6
FRM^    29.8?   31.7?   27.9?
...
- RP2350 はフレーム ≈ `BGN` + 全画面転送 (8.5 + 17.6 = 26.1 ms に対して `FRM` 26.5) で、転送律速のまま。
  帯の中身は 2D (`f2D` 8.5〜9.2 ms: 網の DDA、HUD) が大半で、3D (`f3D`) は 0.6〜2.3 ms。
- **ESP32S3 の `STK0` は 3,360** で、ワイヤーフレーム書き換え後の 3,252 より 108 多い (ba15719 の 3,540 よりは少ない)。残りは 736 バイト。ベンチマーク自体の処理は浅い
  (結果の表の描画は `renderBand` 176 + `drawTextScreen` 112 + `packHead` 48 バイト、xtensa-esp32s3-elf 14.2) ので、
  これはベンチマークの場面 (スフィア 8 の混雑、飛行) でのゲーム本来の経路の値。以後は同じ場面で比べられる。
...
```
BENCHMARK v3.5  XIAMOCON RP2350  240x240 T60  XFR 17.6  STK0 2260  STK1 844
         QUIET   CROWD  FLIGHT
FPS       37.5    33.7    39.9
FRM       26.6    29.6    25.0
FRM^      28.4   33.7?   32.3?
...
```
BENCHMARK v3.5  XIAMOCON ESP32S3  240x240 T30  XFR 25.6  STK0 3360  STK1 1692
         QUIET   CROWD  FLIGHT
FPS       34.5    32.3    35.3
FRM       29.0    30.9    27.9
FRM^      30.0    32.0    30.2
...
- **`STK0` は両ボードとも v3.4 と同じ** (RP2350 2,260、ESP32S3 3,360)。HUD の経路が深くなっても、最深は網の再帰のまま
  (「デバッグ表示」の静的見積りどおり)。
- `f2D` (帯ごとの 2D: 背景・マーカー・線・ゲージ・HUD) が RP2350 で +1.4〜2.4ms、ESP32S3 で FLIGHT だけ +0.9ms。
  FPS は RP2350 の CROWD が −1.4、他は −0.4〜−0.9。
- ゲームが呼ぶ gfx2d の命令数を Cortex-M33 で数えると (unicorn で 1 呼び出しずつ実行)、1 フレームぶんでは差し引き
  ほぼ変わらない (アイコンの多角形が 11,821 → 6,888 と減り、半透明の文字 (HUD の影) が 18,583 → 21,524 と増える)。
  sim を変えていないのに `TCK` も RP2350 で +1.1ms 動いているので、`f2D` の増分の大半はコードの配置
  (XIP / フラッシュキャッシュの当たり方) と見ている。

結果 (2026-09-24、core 3.6。ShapoGFX 4f15411、写真から転記。`textMetrics()` が整数に。このボードは `SUPPRESS_ALPHA` ではないので影は半透明のまま)

```
BENCHMARK v3.6  XIAMOCON RP2350  240x240 T60  XFR 17.6  STK0 2260  STK1 844
         QUIET   CROWD  FLIGHT
FPS       37.1    33.4    40.0
FRM       26.9    29.9    25.0
FRM^     28.5?   33.3?   32.4?
...
```
BENCHMARK v3.6  XIAMOCON ESP32S3  240x240 T30  XFR 25.6  STK0 3360  STK1 1692
         QUIET   CROWD  FLIGHT
FPS       34.6    33.1    36.1
FRM       28.9    30.2    27.7
FRM^      29.8    31.9    30.0
...
- 3.5 とほぼ同じ。`f2D` は RP2350 で −0.1〜0.4ms (FPU があるので整数化の効果は小さい)、v3.4 との差は
  +1.3〜2.0ms 残る。`STK0` は両ボードとも変わらず。
