...
| **合計** | **333,432 (325.6KB / 512KB)** |

`.text` は 289,640 バイト (4MB のフラッシュに対して十分小さい)。
...
| | 三角形 | ピーク使用 | 96KB での空き | 128KB での空き |
|---|---|---|---|---|
| 現在 (4 レイヤ) | 261 | **17,732B** | 70,432B (4.0 倍) | 100,032B (5.6 倍) |

シーンが本来の大きさ (261 三角形) に達するのに要る三角形バッファは
実機換算で約 **38KB**。それを下回ると `buildScene()` の予算が減り、
まずエンティティがフラグメント単位の細かい形を失う (取りこぼしが始まるのはさらに下)。
**現状は 96KB でも変曲点の 1.8 倍、ピークの 4 倍**あり、アリーナは完全に余っている。
...
この余裕は自動的に使われる。`ARENA_SIZE` は据え置いている。

**まだ取っていない余地** (どちらも別途の判断):

- `Config::spanCapacity`。`spanPeak` は 31〜35 しかないのに、既定はアリーナの
  1/4 (96KB で 451 スパン = 23KB) を取る。96 に絞れば 18KB 戻る。
  `Renderer::init()` に通す小さな API 追加が要る。
- **アリーナ自体を 64KB に落とせる** (変曲点の 1.5 倍、ピークの 3 倍)。
  ESP32S3 では DRAM が 32KB 浮く。core1 のスタックが取れずに黙って起動しない、
  というのと同じプールなので、あのボードでは価値がある。
...
実測: **RAM 149,684 / 327,680 (45.7%)** (帯バッファは実行時にヒープから取るので
...
TRI 261 17/97K d0         三角形数 / バッファの使用量と総量 (KB) / 取りこぼし
