...
- **ShapoGFX の更新のたびにシーン構築側のスタックが増えている。**
  ...
  | b03055e | 2,528 | 1,568 |
  | ba15719 | 2,736 | 1,360 |
  | ワイヤーフレーム書き換え後 (現在) | **2,596** | **1,500** |

  ba15719 時点の実機実測 (`STK0`): **RP2350 2,532 / ESP32S3 3,540**。
  RP2350 は静的見積りとよく一致する。ESP32S3 が 1,000 バイト多いのは
  FreeRTOS のタスク切り替えと Xtensa のレジスタウィンドウのぶんで、
  **ここが一番細い**。

  ワイヤーフレームの書き換えで `subdivideFace()` は 216→232 バイトに増えたが、
  弦の分割を再帰ではなくループにし、`traverseSphere()` と `emitIcoEdges()` を
  分けて再帰中に生きるフレームを 32 バイトに抑えたので、最深経路は逆に
  140 バイト縮んだ。ESP32S3 でも 3,400 前後に収まるはずだが、**書き込み時に
  `STK0` を確認すること**。
