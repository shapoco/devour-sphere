...
`.text` は 280,212 バイト (4MB のフラッシュに対して十分小さい)。
8eff034 でラスタライザのコードが増えたぶん約 2.7KB 大きくなっているが、
`.bss` は変わらない (アリーナは固定長の配列で、その中の割り付けが変わるだけ)。
...
アリーナの制約になるのはシーンが使うバイト数 (ピークで 40.6KB) ではなく、
...
| アリーナ | 三角形 | | アリーナ | 三角形 |
|---|---|---|---|---|
| 48KB | 193/251 | | 128KB | 263/717 |
| 64KB | 193/344 | | 160KB | 263/956 |
| 96KB | 230/530 | | 256KB | 263/1700 |
| 104KB | 263/577 | | | |

104KB が変曲点で、ここでシーンが本来の大きさ (263 三角形) に達する。
これ以上増やしても何も変わらない。128KB ならより重い場面に対して 2.7 倍の余裕がある。

ShapoGFX 8eff034 (レンダラの高速化) で `Triangle` が 136→128 バイトになり、
同じアリーナが約 6% 多く入るようになった。変曲点も 112KB から 104KB へ下がっている。
LOD は `triCapacity` から毎フレーム予算を引き直すので、この余裕は自動的に使われる
(ESP32S3 の 96KB ではピークが 212→230 三角形に増えた)。定数の調整は不要。
...
この数字には入らない)、Flash 約 603KB / 3,342,336 (18%)。
...
TRI 140/500 d0              アリーナ 96KB での容量 (8eff034 以降は 530)
...
TRI 263/717 d0            三角形バッファの使用量と取りこぼし
SPN 44 d0 ARN 41K         スパンのピークと取りこぼし、アリーナの実使用量
...
- ShapoGFX 8eff034 でシーン構築側のフレームが増えている
  (`-fstack-usage` 実測: `emitTriangle()` 48→136、`putPrimitive()` 208→248、
  `shadeVertex()` 28→56 バイト。逆にラスタライズ側の `render()` は 256→176 と減った)。
  一番深いのは `buildSphere()` の 7 段再帰から `putLine3()` へ降りる経路で、
  そこでの増分は +40 バイト。余裕はあるはずだが、`STK0` を一度は見ておくこと。
