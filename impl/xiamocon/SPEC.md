...
### `Game` を DRAM に置く構成 (計測用)

`DS_GAME_IN_PSRAM=0` でビルドすると `Game` を内部 DRAM に置く。
PSRAM が tick をどれだけ遅くしているかを測るためのもの。

```sh
PLATFORMIO_BUILD_FLAGS=-DDS_GAME_IN_PSRAM=0 pio run -e esp32s3_arduino -t upload
```

`Game` は 136KB あり DRAM の予算は 327,680 バイトしかないので、この構成では
**アリーナが 48KB、帯が 40 行に戻る**。RAM は 237,028 (72.3%) で、残りは
帯バッファと SPI の DMA バッファがヒープから取る分。ぎりぎりなので、
起動時トレースの `band buffer` が 0 なら確保に失敗している。

アリーナ 48KB は 6 帯のときに測った「取りこぼしが出ない最小」なので、
3 帯でないこの構成では**三角形が落ちる可能性がある** (`TRI` の `d` が 0 以外になる)。
遊ぶための構成ではなく、`TCK` を比べるためのもの。
...
- `DS_RENDER_ON_CORE1=0` にして core0 で描くと、**表示も電源ボタンも正常になる**
