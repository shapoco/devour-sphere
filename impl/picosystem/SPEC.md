...
このターゲットではさらに 4 行、tick と `beginFrame()` の内訳が付く
(core/ の `devoursphere::profileClockUs` に時計を渡すと `Game` と `Renderer` が
フェーズごとの時間を積算する。core/SPEC.md「フェーズ計測」):

```
TK E10.2 B1.0 F2.3     1 tick あたりの内訳 (ms)。E エンティティ (AI・移動・配置物理・発射)
O0.5 K3.1 C4.2 X0.1    B 弾丸  F 浮遊フラグメントとアップグレード  O 近傍順序の再構築
                       K 捕食  C エンティティ同士の衝突  X その他 (メニュー、順位、リスポーン)
BG C1.2 F0.8 S9.1      1 フレームの beginFrame の内訳 (ms)。C カメラ  F エフェクト
O20.1 H1.0 R3.5        S 星とワイヤーフレーム  O エンティティ (マーカー、ソート、本体)
                       H 残り (浮遊フラグメント、弾丸、エフェクト、オーバーレイ)  R 深度ソート
```

`TK` の行はバッチの合計を tick 数で割った値、`BG` の行はフレームあたり。どちらも 1 フレーム遅れ。
