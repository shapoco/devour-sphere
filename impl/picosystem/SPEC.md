...
| **`.bss` 合計** | **201,568** |
| `.data` (gfx3d のコード 48KB を含む) | 54,868 |
| **RAM 合計** | **256,436 (250.4KB / 256KB)** |

フラッシュ側は `.text` + `.rodata` + `.data` の初期値で約 236KB (16MB のうち)。RAM の空きは約 5.7KB。
...
- gfx3d のコード (約 48KB) は RAM に置いている (後述)。
...
`.data` が 54.9KB になる。`-DDS_GFX3D_IN_RAM=OFF` で比較できる。
