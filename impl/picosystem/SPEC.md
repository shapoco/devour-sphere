...
RP2040 では露骨に FPS が落ちたので `DEVOURSPHERE_SPHERE_TARGET_PX=36` にしている。
本数は px の 2 乗に反比例するので、ホストの固定条件 (240x240) での実測は

| 目標 px | 落ち着いた本数 (2^2〜2^20) | 固定条件の平均 |
|---|---|---|
| 24 (既定) | 298〜371 | 302 |
| 30 | 208〜269 | 245 |
| 36 (この版) | 164〜199 | 202 |
...
3.0 は同じ固定条件で平均 122 本だったが、それは**網が崩れていたフレームが多かった**ためで、
地上で落ち着いたときは 240〜377 本だった。36 px は崩れないまま 3.0 の平均に近いところに収まる。
網の見た目は 24 px より粗いが、密度は画面全体で一様のまま。
