...
内蔵 SRAM に移せれば tick は縮むが、内蔵ヒープは帯バッファ 43KB と
各タスクのスタックで既に埋まっているので、アリーナを削らないと入らない。
...
- `Game` を内蔵 SRAM に移すか (上記)。移せれば 50fps 台が見込めるが、
  内蔵ヒープが枯れると帯バッファか core1 のタスクが黙って確保できなくなる
  (Xiamocon の ESP32S3 版で実際に起きた失敗)。
- アリーナ 64KB は `TRI 21/53K` から見て過剰。40KB 程度まで削れる (上と同じ話)。
