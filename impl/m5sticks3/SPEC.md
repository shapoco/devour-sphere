...
正しいかどうかを一目で確かめるためのもので、**DOWN の端は常に床を指していなければならない**。
M5Unified はこのボードに対して軸の補正を持っていないので、この対応は実機でしか決まらない
(**実機で確認済み: BMI270 は画面と同じ向きに載っており、対応は恒等でよい**。2026-09-21)。

この針は**倒した向きと逆に振れる。それが正しい**。画面はデバイスと一緒に回るが重力は回らないので、
画面から見た重力は必ず逆向きに回る (だからこそ床を指し続ける)。
姿勢計のつもりで見ると逆に見えるので、針だけでなく DOWN のラベルを見ること。
間違っているときの見え方は「天井を指す」(x と y が両方反転) か
「鏡像になる = 倒した向きと同じに回る」(どちらか一方が反転) のどちらかになる。
...
書いた時点では実機に触れていなかったので、ソフトウェアからは決まらない 3 つを
起動の 1 回で確かめた。**3 つとも既定のままで正しかった**。

1. **BMI270 の軸の対応**。センサの軸は画面の軸と符号まで一致する。`SRC_* / SIGN_*` は恒等。
2. **`ROTATION_TOP_RIGHT` = 1**。倒すと絵は正しい向きで出る。
3. **KEY2 = GPIO12**。M5Unified 0.2.18 以降の BtnB がそのまま使える
   (製品資料の GPIO42 ではない。直読みへの逃げ道は不要だった)。

## 未確定 (遊んで決めること)

- **左右とダッシュ/ブレーキの符号**。軸の対応と画面の向きが合ったので
  導出どおりなら合っているはずだが、実際に倒して確かめるまでは確定しない。
  左右がおかしければ `updateKeys()` の `c.z` の符号、
  ダッシュ/ブレーキがおかしければ `fwd` の符号だけを反転する。
- **`TICK_RATE`**。60 で始めている。240x135 は Xiamocon の ESP32S3 版 (240x240 で 30Hz、
  core0 が 28.5ms) の 56% の画素数なので届くはずだが、オーバーレイの CPU 行が
  16.6ms を超えるようなら 30 に落とす。
- `SE_MASTER_VOLUME` (既定 64。M5Unified は音量を 2 乗する)、傾きの閾値 15 度、
  振りの閾値 0.70G、帯 45 行 x 3 (オーバーレイの CMD 行が高ければ 27 行 x 5)。
