// Sound effects on the M5StickS3 (the audio:: interface of
...
// pointing a DMA channel at the pack, and the other ESP32S3 board by feeding
// one cursor into an I2S ring, so a second sound would have to take the
// first one's place. Here M5Unified mixes eight channels into the ES8311, in
// its own task, so every sound a tick asks for is played -- which is what
// the browser does and what the game was tuned against. There is no priority
// table for the same reason: nothing has to lose.
//
// The mixer costs CPU that the two cores are otherwise spending on the
// simulation and the rasterizing, so its task is pinned in app_main.cpp
// (M5.Speaker's config) to the core that is not drawing. If the overlay
// ever shows the frame time moving with the sound, that is the knob, and
// below it the handhelds' one-voice arrangement.
//
...
// memory mapped, so playRaw() can read it in place, and M5Unified resamples
// it to the 44.1 kHz the codec is configured for.
