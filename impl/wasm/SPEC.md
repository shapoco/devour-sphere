...
- 画面は既定で 480x320px。RGB565BE のフレームバッファを ShapoGFX で描画し、キャンバスに表示する。
  URL に `#screen=WxH` を付けると別の解像度で起動する (`?screen=WxH` でも可)。
...
| パラメータ | 内容 |
|---|---|
| `#screen=WxH` | フレームバッファの大きさ (既定 480x320)。`?screen=WxH` でも可 |
...
RP2350 版ではフレームバッファを持たずに 2 本の帯バッファを交互に使い、
帯単位で描画してディスプレイへ DMA 転送する構成に置き換える。
