...
- ゲームパッドは `navigator.getGamepads()` を tick ごとにポーリングする (標準マッピング前提)。
  ...
  **斜めに倒しきると両軸とも最大**になる。クイックターンは最大の旋回と最大のブレーキの組み合わせで、
  ...
  にしかならず、ゆっくりしか回らなかった。写した後の各成分は、大きい方の 0.15 倍 (軸から約 8.5 度) までが 0、
  0.6 倍 (約 31 度) から最大で、その間は線形。軸から少しずれて倒しても旋回に弱いダッシュやブレーキが混ざらず、
  斜めから ±14 度以内は両軸とも最大 (キーを 2 つ押したのと同じで、キーボードと同じく 1 周 2 秒で回る)。
  飽和させずに線形のまま写すと、45 度から 15 度ずれただけでブレーキが半分になり、1 周に 2.7 秒かかった。
