...
残りの 2^18 - 95 エントリは、ハミング距離 x 32 + |点灯数の差| が最小の文字で埋める
(同点は文字コードの小さい方)。総当たり 262,144 x 95 で 30 ms 程度、表は 256 KB。
...
`Game::sounds()` のうち `BEEP_MASK` の種類 (HIT_PLAYER、ENEMY_KILLED_*、PLAYER_KILLED、GET_UPGRADE、
MENU_SELECT、MENU_START、LAUNCH、ARRIVE、TIME_ALARM、DODGE) が立っていれば BEL を 1 バイト書く。
100 ms (`BEEP_GAP_US`) に 1 回まで。撃つ・当てる・餌の音は鳴らさない (数フレームおきに要求され、鳴りっぱなしになる)。
音の高さも長さも端末任せで、無音やビジュアルベルの端末も多い。ミュート (タイトル / ポーズの ↓) は
`sounds()` が 0 になるのでそのまま効く。永続化はしない。
