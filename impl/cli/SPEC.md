...
タイトル画面で **B のキーを 3 秒** 押し続けると始まる (core/SPEC.md「ベンチマーク」。legacy のキー入力では
キーリピートで押しっぱなしを推定する)。実行中は 1 tick ごとに 1 フレームを描いて端末に書き、待たない。
`DMA` は文字への変換と端末への書き込み。結果は文字の画面では読みにくいので、終了時 (`q`) に標準出力にも
同じ行を出す。
