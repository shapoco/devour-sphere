#!/usr/bin/env bash
# Flash Devour Sphere onto an M5Stack Tab5 (ESP32-P4).
#
#   ./upload.sh [PORT] [BAUD]
#
...
#
# If the port does not appear, put the board in download mode: hold the
...
set -x
$ESPTOOL --chip esp32p4 --port "$PORT" --baud "$BAUD" "$WRITE" 0x0 "$BIN"
